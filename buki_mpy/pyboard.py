...
def stdout_write_bytes(b):
    ...
    for i in b:
        if i == 0x04:
            return
        elif i >> 7 == 0:  # i is ASCII Code
            stdout.write(i.to_bytes(1, "big"))
            count = 0
        elif i >> 5 == 0b110:  # i is 2bytes char
            stdout.write(i.to_bytes(1, "big"))
            count = 1
        elif i >> 4 == 0b1110:  # i is 3bytes char
            stdout.write(i.to_bytes(1, "big"))
            count = 2
        elif i >> 3 == 0b11110:  # i is 4bytes char
            stdout.write(i.to_bytes(1, "big"))
            count = 3
        else:  # i is in the middle of UTF8
            stdout.write(i.to_bytes(1, "big"))
            count -= 1
        if count == 0:
            stdout.flush()
